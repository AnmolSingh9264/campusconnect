...
export default function EditProfile() {
  ...
  const nextStep = () => setCurrentStep((prev) => Math.min(prev + 1, 4) as OnboardingStep);
    ...
    <div className="flex min-h-screen bg-[#F8FAFC]">
      ...
      <aside className="fixed left-0 top-0 hidden lg:flex h-screen w-[280px] bg-white border-r border-slate-200 flex-col p-8 z-40">
        <div className="flex items-center gap-2 mb-12">
          ...
          <span className="text-xl font-extrabold tracking-tight text-slate-900 font-headline">Hire Skills</span>
        </div>

        <nav className="flex-grow space-y-8">
          <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Your Journey</div>
          <div className="space-y-6">
            <SidebarStep 
              ...
              status={currentStep === 1 ? 'Active Now' : currentStep > 1 ? 'Completed' : 'Upcoming'}
            ...
            <SidebarStep 
              ...
              status={currentStep === 2 ? 'In Progress' : currentStep > 2 ? 'Completed' : 'Upcoming'}
            ...
            <SidebarStep 
              ...
              status={currentStep === 3 ? 'Refining' : currentStep > 3 ? 'Completed' : 'Next Step'}
        ...
        <div className="mt-auto space-y-6 pt-8 border-t border-slate-100">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-slate-900 uppercase tracking-tight">Need help?</p>
              <p className="text-[10px] text-slate-500">View Onboarding Guide</p>
            </div>
          </div>

          <div className="bg-slate-50 rounded-xl p-4 border border-slate-100">
            <div className="flex justify-between items-center mb-2">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">Progress</span>
              <span className="text-[10px] font-bold text-primary">{progressMap[currentStep]}%</span>
            </div>
      ...
      <main className="flex-1 lg:ml-[280px]">
        ...
        <header className="bg-white/70 backdrop-blur-xl border-b border-slate-200/50 sticky top-0 z-50 h-16 w-full px-8 flex items-center justify-between">
          <div className="lg:hidden flex items-center gap-2">
            ...
            <span className="font-bold text-slate-900 tracking-tight">Hire Skills</span>
          </div>

          <div className="hidden md:flex items-center gap-8 text-sm font-medium">
            <button className="text-primary font-bold border-b-2 border-primary py-5">Challenges</button>
            <button className="text-slate-500 hover:text-primary transition-colors">Courses</button>
            <button className="text-slate-500 hover:text-primary transition-colors">Talent Pool</button>
            <button className="text-slate-500 hover:text-primary transition-colors">Pricing</button>
          </div>
        ...
        <div className="p-6 md:p-12 lg:p-16 max-w-5xl mx-auto min-h-[calc(100vh-64px)] relative overflow-hidden">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              {currentStep === 2 && <FeaturedProject onNext={nextStep} onPrev={prevStep} />}
              {currentStep === 3 && <CareerGoalsView onNext={nextStep} onPrev={prevStep} />}
...
function SidebarStep({ icon, label, active, completed, status, onClick }: { 
  ...
  return (
    ...
    >
      ...
      <div className="text-left">
        <p className={`text-sm font-bold transition-colors ${active ? 'text-primary' : 'text-slate-600'}`}>{label}</p>
        <p className={`text-[10px] font-medium tracking-wide uppercase transition-colors 
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <header className="mb-10">
        ...
        <h1 className="text-4xl font-extrabold text-slate-900 mb-3 tracking-tight">Let's start with the basics</h1>
        <p className="text-slate-500 text-lg">Tell us a bit about yourself so we can personalize your career journey and connect you with the right opportunities.</p>
      </header>
      ...
      <div className="glass-panel rounded-3xl p-8 md:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] ring-1 ring-slate-200/50">
        <form className="space-y-8" onSubmit={(e) => { e.preventDefault(); onNext(); }}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-7">
            <FormField label="Full Name" icon={<User className="w-5 h-5" />} placeholder="John Doe" />
            <FormField label="Email Address" icon={<Mail className="w-5 h-5" />} placeholder="john.doe@example.com" type="email" />
            <SelectField label="Gender" icon={<Wc className="w-5 h-5" />} options={['Male', 'Female', 'Other']} />
            <SelectFieldUp
  id="university"
  label="University"
...
/>
           ...
           <div className="space-y-1.5">
            ...
            <div className="md:col-span-2">
              <SelectField label="Graduation Year" icon={<Calendar className="w-5 h-5" />} options={['2023', '2024', '2025', '2026', '2027']} />
            </div>
          </div>

          <div className="pt-6 flex justify-end items-center gap-6 border-t border-slate-100">
            <button type="button" className="text-sm font-bold text-slate-500 hover:text-primary transition-colors px-4 py-2">
              Save Draft
            </button>
            <button 
              type="submit" 
      ...
      <div className="mt-10 flex items-start gap-4 p-5 bg-indigo-50/50 rounded-2xl border border-indigo-100 ring-1 ring-white">
        ...
        <p className="text-xs text-indigo-700/80 leading-relaxed font-medium">
          Completing your profile increases your visibility to top-tier tech recruiters by up to <span className="font-bold text-primary">3x</span>. Ensure your university and graduation year are accurate to receive relevant internship and job alerts.
        </p>
      </div>
    </div>
  );
...
function FeaturedProject({ onNext, onPrev }: { onNext: () => void, onPrev: () => void }) {
    ...
    <div className="max-w-5xl mx-auto">
      <header className="mb-10 flex flex-col md:flex-row justify-between items-end gap-6">
        <div className="max-w-2xl">
          <h2 className="text-4xl font-extrabold tracking-tight text-slate-900 mb-3">Showcase your best work</h2>
          <p className="text-slate-500 text-lg leading-relaxed">
            Highlight a project that demonstrates your technical proficiency and problem-solving skills. This is the first thing hiring managers will see on your profile.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-12 xl:col-span-7 space-y-8">
          <div className="glass-panel p-8 md:p-10 rounded-[32px] border-slate-200 shadow-sm space-y-7">
            <FormField label="Project Title" placeholder="e.g. Distributed Ledger for FinTech" />
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700 ml-1">GitHub Repository Link</label>
              <div className="relative group">
            ...
            <div className="space-y-3">
              <label className="text-sm font-bold text-slate-700 ml-1">Key Technologies</label>
              <div className="flex flex-wrap gap-2.5">
                {tags.map(tag => (
                  <span key={tag} className="px-4 py-2 bg-indigo-50 text-indigo-700 rounded-xl text-xs font-bold flex items-center gap-2 border border-indigo-100 transition-all hover:scale-105">
                    {tag} <X className="w-3.5 h-3.5 cursor-pointer hover:text-red-500" />
                  </span>
                ))}
                <button className="px-4 py-2 border-2 border-dashed border-slate-200 rounded-xl text-xs font-bold text-primary hover:border-primary hover:bg-primary/5 transition-all flex items-center gap-1.5" type="button">
          ...
          <div className="glass-panel p-7 rounded-[32px] border-slate-200 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium">Do you have a deployed version?</p>
              </div>
            </div>
        ...
        <div className="lg:col-span-12 xl:col-span-5 space-y-6">
          <div className="glass-panel overflow-hidden rounded-[32px] border-slate-200 shadow-sm flex flex-col h-full bg-white/50">
            <div className="p-7 border-b border-slate-100 bg-slate-50/50">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">Project Thumbnail</h3>
              <p className="text-[10px] text-slate-400 mt-1 font-medium italic">Recommended size: 1600x900px (16:9)</p>
            </div>
            <div className="flex-1 min-h-[300px] p-8">
              <div className="relative h-full aspect-video rounded-2xl border-2 border-dashed border-slate-200 bg-white/50 flex flex-col items-center justify-center group hover:border-primary transition-all cursor-pointer hover:bg-white shadow-inner">
                ...
                <p className="z-10 mt-5 font-bold text-slate-800 text-sm">Click to upload or drag & drop</p>
                <p className="z-10 text-[11px] text-slate-400 mt-1 font-medium uppercase tracking-tight">SVG, PNG, JPG (MAX. 5MB)</p>
              </div>
            </div>
            <div className="p-7 bg-indigo-50/30 border-t border-slate-50">
              <div className="flex items-start gap-4">
                ...
                <p className="text-xs leading-relaxed text-slate-500 font-medium italic">
                  <span className="font-bold text-primary not-italic">Pro Tip:</span> Projects with high-quality thumbnails receive up to <span className="font-bold text-primary underline underline-offset-2">40% more</span> engagement from technical recruiters.
                </p>
              </div>
            </div>
          </div>
...
function CareerGoalsView({ onNext, onPrev }: { onNext: () => void, onPrev: () => void }) {
  return (
    <div className="max-w-4xl mx-auto pb-20">
      <header className="mb-12">
        <h2 className="text-4xl font-extrabold text-slate-900 mb-4 tracking-tight">Define your career vision.</h2>
        <p className="text-lg text-slate-500 max-w-2xl leading-relaxed">
          We use your goals to match you with elite opportunities that align with your professional trajectory and financial expectations.
        </p>
      </header>
      ...
      <div className="space-y-10">
        <section className="bg-white rounded-[32px] p-8 md:p-10 border border-slate-200 shadow-[0_10px_30px_rgba(15,23,42,0.03)] ring-1 ring-slate-100">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h3 className="text-xl font-bold text-slate-800">Desired Job Roles</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <SelectionCard icon={<Layers className="w-5 h-5" />} title="Product Architect" subtitle="System design & strategy" selected />
            <SelectionCard icon={<History className="w-5 h-5" />} title="Lead Engineer" subtitle="Technical team leadership" selected />
            <SelectionCard icon={<TargetIcon className="w-5 h-5" />} title="Data Strategist" subtitle="Insight-driven growth" />
          </div>
          <button className="mt-8 text-primary text-sm font-bold flex items-center gap-2 hover:gap-3 transition-all" type="button">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <section className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-[0_10px_30px_rgba(15,23,42,0.03)] flex flex-col">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-xl font-bold text-slate-800">Work Environment</h3>
            </div>
            <div className="space-y-5">
              <button className="w-full flex items-center justify-between p-5 rounded-2xl border-2 border-primary bg-indigo-50/50 shadow-sm group">
                <div className="flex items-center gap-5">
                  ...
                  <div className="text-left">
                    <span className="block font-bold text-slate-900">Remote First</span>
                    <span className="text-xs text-slate-500 font-medium">Global collaboration</span>
                  </div>
                </div>
              ...
              <button className="w-full flex items-center justify-between p-5 rounded-2xl border-2 border-slate-100 hover:border-slate-200 transition-all hover:bg-slate-50 group">
                <div className="flex items-center gap-5">
                  ...
                  <div className="text-left">
                    <span className="block font-bold text-slate-800 transition-colors group-hover:text-slate-900">On-Site / Hybrid</span>
                    <span className="text-xs text-slate-400 font-medium transition-colors group-hover:text-slate-500">In-office presence</span>
                  </div>
                </div>
              </button>
            </div>
          ...
          <section className="bg-white rounded-[32px] p-8 border border-slate-200 shadow-[0_10px_30px_rgba(15,23,42,0.03)] flex flex-col">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h3 className="text-xl font-bold text-slate-800">Annual Salary</h3>
            </div>
            <div className="flex-1 flex flex-col justify-center">
              <div className="flex items-end justify-between mb-8">
                <div className="text-center">
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Minimum</p>
                  <p className="text-3xl font-extrabold text-slate-900">$140k</p>
                </div>
                <div className="text-center">
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1">Maximum</p>
                  <p className="text-3xl font-extrabold text-slate-900">$210k+</p>
                </div>
              </div>
              ...
              <p className="text-xs text-center text-slate-400 font-medium italic border-t border-slate-50 pt-4">
                Based on your experience, this is the <span className="text-primary font-bold">top 5%</span> of your peer group.
              </p>
            </div>
          </section>
        </div>
        ...
        <section className="glass-panel rounded-[32px] p-8 md:p-10 border border-primary/10 shadow-xl overflow-hidden relative group">
          <div className="relative z-10 w-full">
            <h4 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
              <TargetIcon className="w-5 h-5 text-primary" /> Professional Ambition
            </h4>
        ...
        <footer className="flex items-center justify-between pt-12">
          <button onClick={onPrev} className="px-8 py-3.5 rounded-full text-slate-500 font-bold hover:bg-slate-100 transition-all flex items-center gap-2">
            <ArrowLeft className="w-5 h-5" /> Previous
          </button>
          <button onClick={onNext} className="primary-gradient px-12 py-4 rounded-full text-white font-extrabold shadow-2xl transition-all flex items-center gap-3 group">
            Continue Journey
  ...
  return (
    <div className="max-w-[1000px] mx-auto pb-20">
      <header className="mb-14">
        ...
        <h2 className="text-5xl font-extrabold text-slate-900 mb-5 tracking-tighter">Tell us what drives you.</h2>
        <p className="text-xl text-slate-500 max-w-2xl leading-relaxed">
          We use these insights to match you with teams that share your values and challenges that test your unique strengths.
        </p>
      </header>
      ...
      <div className="grid grid-cols-12 gap-8">
        <section className="col-span-12 glass-panel border border-slate-200 p-10 rounded-[40px] shadow-sm">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h3 className="text-2xl font-bold text-slate-900">Soft Skills & Power Skills</h3>
              <p className="text-sm text-slate-400 font-medium">Interpersonal strengths you bring to every project.</p>
            </div>
          </div>
        ...
        <section className="col-span-12 lg:col-span-7 glass-panel border border-slate-200 p-10 rounded-[40px] shadow-sm">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h3 className="text-2xl font-bold text-slate-900">Industry Interests</h3>
              <p className="text-sm text-slate-400 font-medium">Which sectors excite you most?</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-5">
            <IndustryItem icon={<CreditCard className="w-6 h-6" />} label="FinTech" selected />
            <IndustryItem icon={<BrainCircuit className="w-6 h-6" />} label="AI & ML" selected />
            <IndustryItem icon={<School className="w-6 h-6" />} label="EdTech" />
            <IndustryItem icon={<Stethoscope className="w-6 h-6" />} label="HealthTech" />
            <IndustryItem icon={<TreePine className="w-6 h-6" />} label="ClimateTech" />
            <IndustryItem icon={<SpaceIcon className="w-6 h-6" />} label="SpaceTech" />
          </div>
        </section>
        ...
        <section className="col-span-12 lg:col-span-5 glass-panel border border-slate-200 p-10 rounded-[40px] shadow-sm flex flex-col">
          <div className="flex items-center gap-4 mb-8">
            ...
            <div>
              <h3 className="text-2xl font-bold text-slate-900">Preferred Team Size</h3>
              <p className="text-sm text-slate-400 font-medium">Where do you perform best?</p>
            </div>
          </div>
          <div className="space-y-6 flex-1 flex flex-col">
            <div className="space-y-3">
              ...
              <input className="w-full h-2.5 bg-slate-100 rounded-lg appearance-none cursor-pointer accent-primary" type="range" min="0" max="100" defaultValue="40" />
            </div>
            <div className="mt-6 flex-1 flex flex-col justify-center">
              <div className="p-5 rounded-3xl bg-white border-2 border-primary shadow-sm flex items-center justify-between mb-4">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-indigo-50 flex items-center justify-center text-primary font-bold text-lg">10-50</div>
                  <div>
                    <p className="font-extrabold text-slate-900">Scale-up</p>
                    <p className="text-[11px] text-slate-400 font-bold italic uppercase tracking-tighter">Fast-paced growth phase</p>
                  </div>
                </div>
              ...
              <div className="p-5 rounded-3xl bg-slate-50 border border-slate-100 flex items-center justify-between opacity-50 grayscale">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-2xl bg-white border border-slate-200 flex items-center justify-center text-slate-400 font-bold text-lg">500+</div>
                  <div>
                    <p className="font-extrabold text-slate-400">Enterprise</p>
                    <p className="text-[11px] text-slate-400 font-bold italic uppercase tracking-tighter">Established processes</p>
                  </div>
                </div>
      ...
      <footer className="mt-16 pt-10 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
        <button onClick={onPrev} className="flex items-center gap-3 px-8 py-4 rounded-2xl font-bold text-slate-500 hover:bg-slate-100 transition-all w-full md:w-auto">
...
function FormField({ label, icon, placeholder, type = 'text' }: { label: string, icon?: React.ReactNode, placeholder: string, type?: string }) {
  return (
    <div className="space-y-2.5">
      ...
      <div className="relative group">
        ...
        <input type={type} className={`w-full ${icon ? 'pl-12' : 'pl-5'} pr-5 py-4 rounded-2xl border border-slate-200 bg-white/60 focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all text-slate-800 placeholder:text-slate-400 outline-none shadow-sm`} placeholder={placeholder} />
      </div>
    </div>
  );
}
...
function SelectField({ label, icon, options }: { label: string, icon: React.ReactNode, options: string[] }) {
  return (
    <div className="space-y-2.5">
      <label className="text-sm font-bold text-slate-700 ml-1">{label}</label>
      <div className="relative group">
        <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-primary transition-colors z-10 pointer-events-none">{icon}</div>
        <select className="w-full pl-12 pr-12 py-4 rounded-2xl border border-slate-200 bg-white/60 focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all text-slate-800 appearance-none outline-none shadow-sm font-medium">
...
function SelectionCard({ icon, title, subtitle, selected }: { icon: React.ReactNode, title: string, subtitle: string, selected?: boolean }) {
  return (
    <label className="relative cursor-pointer group">
      <input type="checkbox" defaultChecked={selected} className="peer sr-only" />
      <div className="h-full p-5 border-2 border-slate-100 rounded-3xl transition-all peer-checked:border-primary peer-checked:bg-indigo-50/50 group-hover:bg-slate-50/80 group-hover:border-slate-200 flex flex-col gap-3 shadow-sm">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${selected ? 'bg-white text-primary shadow-sm border border-indigo-100' : 'bg-slate-100 text-slate-400'}`}>{icon}</div>
        <div>
          <p className="font-extrabold text-slate-900 leading-tight">{title}</p>
          <p className="text-[11px] text-slate-400 font-bold uppercase tracking-tight italic mt-0.5">{subtitle}</p>
        </div>
        {selected && <div className="absolute top-4 right-4 text-primary"><CheckCircle2 className="w-5 h-5 fill-current" /></div>}
      </div>
    </label>
  );
}
...
function SkillBadge({ label, active }: { label: string, active?: boolean }) {
  return (
    <button className={`px-6 py-3.5 rounded-3xl text-sm font-extrabold flex items-center gap-3 transition-all active:scale-95 shadow-sm ${active ? 'bg-primary text-white shadow-xl shadow-primary/20 scale-105' : 'bg-white border border-slate-200 text-slate-500 hover:bg-slate-50 hover:border-slate-300'}`}>
      {label}
      {active && <X className="w-4 h-4 text-primary-container bg-white rounded-full p-0.5" />}
    </button>
  );
}

function IndustryItem({ icon, label, selected }: { icon: React.ReactNode, label: string, selected?: boolean }) {
  return (
    <label className="relative flex items-center p-5 rounded-3xl border border-slate-200 bg-white cursor-pointer transition-all hover:shadow-lg group overflow-hidden">
      <input type="checkbox" defaultChecked={selected} className="hidden peer" />
      <div className="peer-checked:bg-primary/5 absolute inset-0 transition-all opacity-0 peer-checked:opacity-100" />
      <div className={`relative z-10 w-12 h-12 rounded-2xl flex items-center justify-center transition-all mr-4 ${selected ? 'bg-white text-primary shadow-sm border border-indigo-100' : 'bg-slate-50 text-slate-400'}`}>{icon}</div>
      <span className={`relative z-10 font-extrabold text-base transition-all ${selected ? 'text-slate-900' : 'text-slate-400'}`}>{label}</span>
      {selected && <CheckCircle2 className="relative z-10 ml-auto w-6 h-6 text-primary" />}
    </label>
  );
}
